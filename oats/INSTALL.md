...
|---|---|---|---|---|
| Go | 1.24.2 | `go version` | `brew install go` | `sudo apt-get install -y golang-go` (verify version) or download from https://go.dev/dl/ |
...
`curl | sh`. If Go is below 1.24.2, **stop** and ask the user to upgrade.
...
Default to `dev`. Other reasonable picks: `main` (stable lag), or any
branch the user names explicitly.

```bash
git checkout dev
git branch --show-current   # gate
```
