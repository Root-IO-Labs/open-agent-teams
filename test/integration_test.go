...
// setupIntegrationTest creates a complete test environment with daemon and paths.
// Returns CLI, daemon, session name, and cleanup function.
func setupIntegrationTest(t *testing.T, repoName string) (*cli.CLI, *daemon.Daemon, string, func()) {
	...
	// Session name (backend handles session creation when agents start)
	sessionName := "oat-" + repoName
	...
	return c, d, sessionName, cleanup
}
...
func TestWorkerCreationIntegration(t *testing.T) {
	...
	c, d, sessionName, cleanup := setupIntegrationTest(t, repoName)
	defer cleanup()
	...
	if agent.WindowName != workerName {
		t.Errorf("Agent window name = %s, want %s", agent.WindowName, workerName)
	}
	...
	// Verification 3: In test mode (OAT_TEST_MODE=1), agent startup and
	// window creation are skipped (backend creates windows atomically with
	// agent start). Verify agent is registered in state instead.
...
func TestRepoInitializationIntegration(t *testing.T) {
	// Set test mode to skip Agent startup
	os.Setenv("OAT_TEST_MODE", "1")
	defer os.Unsetenv("OAT_TEST_MODE")
	...
	if repo.SessionName != sessionName {
		t.Errorf("Repo session name = %s, want %s", repo.SessionName, sessionName)
	}
	...
	// Verification 3: Session management is handled by the backend.
	// In test mode, sessions are not actually created (backend creates them
	// atomically with agent start, which is skipped).
	...
	// Verification 6: In test mode, windows are not created (backend creates them
	// atomically with agent start, which is skipped in test mode).
	// Verify agents are registered in state instead.
}
...
func TestDaemonCommunicationRoundTrip(t *testing.T) {
	...
	_, d, sessionName, cleanup := setupIntegrationTest(t, repoName)
	defer cleanup()
...
// TestMultiRepoIsolation verifies that multiple repos tracked simultaneously
// have completely isolated agents, messages, and state.
func TestMultiRepoIsolation(t *testing.T) {
	_, d, _, cleanup := setupIntegrationTest(t, "repo-alpha")
	defer cleanup()

	client := socket.NewClient(d.GetPaths().DaemonSock)

	// Set up two repos with git directories
	for _, repoName := range []string{"repo-alpha", "repo-beta"} {
		repoPath := filepath.Join(d.GetPaths().ReposDir, repoName)
		setupTestGitRepo(t, repoPath)
		...
		resp, err := client.Send(socket.Request{
			...
			Args: map[string]interface{}{
				"name":         repoName,
				"github_url":   "https://github.com/test/" + repoName,
				"session_name": "oat-" + repoName,
			},
		})
		if err != nil {
			t.Fatalf("Failed to add %s: %v", repoName, err)
		}
		if !resp.Success {
			t.Fatalf("Failed to add %s: %s", repoName, resp.Error)
		}
	}
	...
	for _, repoName := range []string{"repo-alpha", "repo-beta"} {
		resp, err := client.Send(socket.Request{
			...
			Args: map[string]interface{}{
				...
				"worktree_path": filepath.Join(d.GetPaths().WorktreesDir, repoName, "worker-1"),
			},
		})
		if err != nil {
			t.Fatalf("Failed to add agent to %s: %v", repoName, err)
		}
		if !resp.Success {
			t.Fatalf("Failed to add agent to %s: %s", repoName, resp.Error)
		}
	}

	// Verify each repo lists only its own agent
	for _, repoName := range []string{"repo-alpha", "repo-beta"} {
		resp, err := client.Send(socket.Request{
			Command: "list_agents",
		...
		if err != nil {
			t.Fatalf("Failed to list agents for %s: %v", repoName, err)
		}
		agents, ok := resp.Data.([]interface{})
		if !ok {
			t.Fatalf("Expected []interface{} for %s agents, got %T", repoName, resp.Data)
		}
		if len(agents) != 1 {
			t.Errorf("%s: expected 1 agent, got %d", repoName, len(agents))
		}
	}

	// Complete agent in repo-alpha; verify repo-beta is unaffected
	resp, err := client.Send(socket.Request{
		Command: "complete_agent",
		Args:    map[string]interface{}{"repo": "repo-alpha", "agent": "worker-1"},
	})
	if err != nil || !resp.Success {
		t.Fatalf("Failed to complete agent: %v / %s", err, resp.Error)
	}

	// repo-beta agent should still be active
	st := d.GetState()
	betaAgent, exists := st.GetAgent("repo-beta", "worker-1")
	if !exists {
		t.Fatal("repo-beta worker-1 should still exist")
	}
	if betaAgent.ReadyForCleanup {
		t.Error("repo-beta worker should NOT be marked for cleanup when repo-alpha worker completes")
	}

	// repo-alpha agent should be marked for cleanup
	alphaAgent, exists := st.GetAgent("repo-alpha", "worker-1")
	if !exists {
		t.Fatal("repo-alpha worker-1 should still exist (marked for cleanup, not yet removed)")
	}
	if !alphaAgent.ReadyForCleanup {
		t.Error("repo-alpha worker should be marked for cleanup after completion")
	}
}
