...
// to the per-repo agents directory during `oat init`.
...
func TestAgentDefinitionMerging(t *testing.T) {
	...
	if workerDef, ok := defMap["worker"]; ok {
		...
		if !strings.Contains(workerDef.Content, "oat agent complete") {
...
// TestAgentsListCommand verifies that `oat agents list` shows available definitions.
func TestAgentsListCommand(t *testing.T) {
	...
	if err := st.AddRepo(repoName, &state.Repository{
		...
		SessionName: "oat-list-test-repo",
...
// TestAgentsResetCommand verifies that `oat agents reset` restores defaults.
func TestAgentsResetCommand(t *testing.T) {
	...
	if err := st.AddRepo(repoName, &state.Repository{
		...
		SessionName: "oat-reset-test-repo",
		Agents:      make(map[string]state.Agent),
	}); err != nil {
		t.Fatalf("Failed to add repo: %v", err)
	}
...
// TestAgentsSpawnCommand verifies that `oat agents spawn` creates an agent
...
`
	...
	// In test mode (OAT_TEST_MODE=1), agent startup and window creation are
	// skipped (backend creates windows atomically with agent start).
	// Agent registration in state was already verified above.
}
