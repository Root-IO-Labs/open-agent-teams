...
// TestOrphanedSessionCleanup tests that orphaned sessions are detected via the backend
func TestOrphanedSessionCleanup(t *testing.T) {
	...
	// Create an "orphaned" session via backend (not tracked in state)
	orphanSession := "oat-orphan-test"
	if err := b.CreateSession(ctx, orphanSession); err != nil {
		t.Fatalf("Failed to create orphan session: %v", err)
	}
	defer b.DestroySession(ctx, orphanSession)

	// Verify session exists
	exists, err := b.HasSession(ctx, orphanSession)
	if err != nil {
...
// With DirectBackend (in-memory), sessions don't survive daemon restarts,
// but the state file should be recovered.
func TestDaemonCrashRecovery(t *testing.T) {
	...
	// With DirectBackend, sessions are in-memory and don't survive restarts,
	// but the state file (repos, agents) should be recovered.
	...
	// Verify state was recovered from the state file
...
// in agent prompts rather than via .oat/AGENTS.md
func TestSlashCommandsEmbeddedInPrompts(t *testing.T) {
	...
	// Verify oat CLI commands are referenced
	expectedCLICommands := []string{"oat repo list", "oat worker list", "oat message list"}
