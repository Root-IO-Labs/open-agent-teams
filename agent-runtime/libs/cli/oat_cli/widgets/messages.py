...
from oat_cli.input import EMAIL_PREFIX_PATTERN, INPUT_HIGHLIGHT_PATTERN
from oat_cli.tool_display import format_tool_display
from oat_cli.widgets._links import open_style_link
from oat_cli.widgets.diff import format_diff_textual
...
class ToolCallMessage(Vertical):
    ...
    def set_error(self, error: str) -> None:
        ...
        Args:
            error: Error message (will be escaped to prevent Rich markup crashes)
        """
        self._stop_animation()
        self._status = "error"
        ...
        if command and isinstance(command, str) and command.strip():
            self._output = f"$ {escape_markup(command)}\n\n{safe_error}"
        else:
            self._output = safe_error
        if self._status_widget:
            self._status_widget.remove_class("pending")
            self._status_widget.add_class("error")
    ...
    def _safe_update(self, widget: Static, content: str) -> None:
        """Update a Static widget, falling back to escaped plain text on MarkupError."""
        try:
            widget.update(content)
        except Exception:
            widget.update(Text(escape_markup(content)))
    ...
    def _update_output_display(self) -> None:
        ...
        if self._expanded:
            ...
            self._safe_update(self._full_widget, prefixed)
            ...
            if needs_truncation:
                ...
                prefixed = self._prefix_output(result.content)
                self._safe_update(self._preview_widget, prefixed)
