"""Textual UI application for oat-cli."""
...
from oat_cli.model_config import ModelSpec, save_recent_model
from oat_cli.textual_adapter import TextualUIAdapter, execute_task_textual
from oat_cli.widgets.approval import ApprovalMenu
from oat_cli.widgets.chat_input import ChatInput
from oat_cli.widgets.loading import LoadingWidget
from oat_cli.widgets.message_store import (
    MessageData,
    MessageStore,
    MessageType,
    ToolStatus,
...
from oat_cli.widgets.status import StatusBar
...
class TextualTokenTracker:
    """Path A: Current context window tracker (overwrite semantics).

    Tracks the current conversation's context window size for ``/tokens``
    and compaction decisions.  Each main-agent turn **overwrites** the
    value — this answers "how full is the context window right now?".

    Resettable by ``/clear`` (new conversation = new context).
    Never emitted to the daemon.
    """
    ...
    def add(self, total_tokens: int) -> None:
        """Overwrite the current context window size.

        Args:
            total_tokens: Context window tokens (input + output) for the
                latest main-agent turn.
        """
        self.current_context = total_tokens
        self._update_callback(self.current_context)

    def reset(self) -> None:
        """Reset token count (e.g. on /clear or thread switch)."""
        self.current_context = 0
...
class TokenSpendAccumulator:
    """Path B: Cumulative token spend tracker (accumulate semantics).

    Tracks lifetime token spend for this agent across *all* model
    invocations — including summarization, interrupted work, and failed
    work.  Accumulates with ``+=``, never reset by ``/clear``.

    Feeds ``_emit_oat_tokens()`` for daemon/TUI reporting.

    Model execution is provider-agnostic; token accounting is best-effort.
    If a provider reports no ``usage_metadata``, tracking is skipped.
    If a provider reports usage in an unverified chunk shape, totals may
    be inaccurate until that provider path is validated.  Local/Ollama
    providers are supported for execution even if they do not report
    tokens — missing ``usage_metadata`` is a reporting gap, not a
    runtime failure.
    """
    ...
    def __init__(self) -> None:
        ...
        # Cache metrics: cumulative prompt-cache activity across the session.
        # Populated only for providers that report input_token_details
        # (Anthropic, some others). Stays 0 elsewhere.
    ...
    def record_turn(
        ...
        """Accumulate spend from a completed model invocation."""
...
_COMMAND_URLS: dict[str, str] = {
    "/changelog": "https://github.com/Root-IO-Labs/open-agent-teams/blob/main/libs/cli/CHANGELOG.md",
    "/docs": DOCS_URL,
    "/feedback": "https://github.com/Root-IO-Labs/open-agent-teams/issues/new/choose",
}
...
**Global** (`~/.oat/agents/agent/AGENTS.md`): Universal preferences across all projects
**Project** (`.oat/AGENTS.md`): Project-specific conventions and decisions
...
### Skill Location
`~/.oat/agents/agent/skills/<skill-name>/SKILL.md`
...
class OatSdksApp(App):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        """Initialize the OAT application.
    ...
    async def on_mount(self) -> None:
        ...
        # Create token tracker (Path A: context window) that updates status bar
        self._token_tracker = TextualTokenTracker(
            self._update_tokens, self._hide_tokens
        )

        # Create spend tracker (Path B: cumulative spend) for daemon reporting.
        # NOT reset by /clear — spend is spend.
        self._spend_tracker = TokenSpendAccumulator()
        ...
        if self._agent:
            ...
            self._ui_adapter.set_spend_tracker(self._spend_tracker)
    ...
    async def _handle_command(self, command: str) -> None:
        ...
        if cmd in {"/quit", "/q"}:
            ...
            try:
                from oat_cli._version import (
                    __version__ as cli_version,
                )

                cli_line = f"oat-cli version: {cli_version}"
            except ImportError:
                logger.debug("oat_cli._version module not found")
                cli_line = "oat-cli version: unknown"
            except Exception:
            ...
            try:
                ...
                sdk_version = _pkg_version("oat_sdk")
                sdk_line = f"oat_sdk (SDK) version: {sdk_version}"
            except PackageNotFoundError:
                logger.debug("oat_sdk SDK package not found in environment")
                sdk_line = "oat_sdk (SDK) version: unknown"
            except Exception:
    ...
    async def _handle_compact(self) -> None:
        ...
        try:
            ...
            from oat_sdk.middleware.summarization import (
                SummarizationEvent,
    ...
    async def _run_agent_task(self, message: str) -> None:
        ...
        try:
            await execute_task_textual(
                ...
                backend=self._backend,
                image_tracker=self._image_tracker,
                conversation_log_path=os.environ.get("OAT_TOOL_LOG"),
            )
    ...
    async def _show_thread_selector(self) -> None:
        ...
        from oat_cli.sessions import get_cached_threads, get_thread_limit
    ...
    async def _switch_model(self, model_spec: str) -> None:
        ...
        from oat_cli.agent import create_cli_agent
        ...
        if provider and has_provider_credentials(provider) is False:
            ...
            if env_var:
                ...
                detail = (
                    ...
                    "Add it to ~/.oat/config.toml with an api_key_env field"
                )
    ...
    async def _set_default_model(self, model_spec: str) -> None:
        ...
        Updates `[models].default` in `~/.oat/config.toml` so that
        ...
        if save_default_model(model_spec):
            ...
            await self._mount_message(
                ErrorMessage(
                    "Could not save default model. Check permissions for ~/.oat/"
                )
            )
    ...
    async def _clear_default_model(self) -> None:
        ...
        """
        from oat_cli.model_config import clear_default_model
