...
async def list_threads_command(
    ...
    """CLI handler for `oat_sdk threads list`.
    ...
    from oat_cli.config import COLORS, console
    ...
    if not threads:
        ...
        console.print("[dim]Start a conversation with: oat_sdk[/dim]")
        return
...
async def delete_thread_command(thread_id: str) -> None:
    """CLI handler for: oat_sdk threads delete."""
...
async def prune_old_threads(
    ...
    """Delete checkpoint rows older than `days` days when sessions.db exceeds `max_size_bytes`.

    Runs VACUUM after deletion to reclaim disk space. Safe to call at any time;
    no-ops if the file is small enough. Intended to be called from bench.sh
    before each benchmark run so the DB never grows back to multi-GB size.
    """
    db_path = get_db_path()
    ...
    logger.info(
        "sessions.db is %d MB — pruning threads older than %d days",
    ...
    async with _connect() as conn:
        ...
        # Delete threads whose most recent checkpoint is older than the cutoff.
        # Threads with NULL updated_at are also deleted (incomplete/corrupt rows).
        cursor = await conn.execute(
            """
            ...
            WHERE thread_id IN (
                ...
                GROUP BY thread_id
                HAVING
                    MAX(json_extract(metadata, '$.updated_at')) < datetime('now', ?)
                    OR MAX(json_extract(metadata, '$.updated_at')) IS NULL
            )
            """,
            (f"-{days} days",),
        )
        ...
        # VACUUM reclaims the freed pages — without it the file stays large.
        await conn.execute("VACUUM")
        await conn.commit()

    new_size = db_path.stat().st_size
    print(
        f"sessions.db pruned: {current_size // 1_000_000} MB → {new_size // 1_000_000} MB "
        f"({deleted_rows} rows removed)"
    )
    logger.info(
        "sessions.db: %d MB → %d MB after pruning %d rows",
        current_size // 1_000_000,
        new_size // 1_000_000,
        deleted_rows,
    )
