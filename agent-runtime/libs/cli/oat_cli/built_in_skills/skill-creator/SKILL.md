...
license: MIT
compatibility: designed for oat_sdk-cli
---
...
### Skill Location for OAT CLI

The oat_sdk CLI loads skills from five sources, listed here from lowest to highest precedence:

| # | Directory | Scope | Notes |
|---|-----------|-------|-------|
| 0 | `<package>/built_in_skills/` | Built-in | Ships with oat_sdk CLI |
| 1 | `~/.oat/agents/<agent>/skills/` | User (OAT) | Default for `oat-agent skills create` |
| 2 | `~/.agents/skills/` | User | Shared across agent tools |
| 3 | `.oat/skills/` | Project (OAT) | Default for `oat-agent skills create --project` |
...
```
~/.oat/agents/agent/skills/     # user skills (lowest precedence)
├── skill-name-1/
│   └── SKILL.md
└── ...

<project-root>/.oat/skills/   # project skills (higher precedence)
├── skill-name-2/
│   └── SKILL.md
└── ...
```
...
For oat_sdk CLI, use any of the skill directories listed in "Skill Location for OAT CLI" above:

```bash
# User skills (default)
scripts/init_skill.py <skill-name> --path ~/.oat/agents/agent/skills

# Project skills
scripts/init_skill.py <skill-name> --path .oat/skills
```
...
#### Option B: `oat_sdk skills create` (quick start)
...
oat_sdk skills create <skill-name>

# Create in project skills directory
oat_sdk skills create <skill-name> --project
```

Use `init_skill.py` when the skill will include bundled resources (`scripts/`, `references/`, `assets/`). Use `oat_sdk skills create` for a quick, minimal starting point.
