"""Main entry point and CLI loop for OAT CLI."""
...
def check_cli_dependencies() -> None:
    ...
    if missing:
        ...
        print("\nThe following packages are required to use the OAT CLI:")  # noqa: T201  # CLI output for missing dependencies
        for pkg in missing:
        ...
        print("  pip install oat-cli")  # noqa: T201  # CLI output for missing dependencies
...
def check_optional_tools(*, config_path: Path | None = None) -> list[str]:
    ...
    Args:
        config_path: Path to config file.
            ...
            Defaults to `~/.oat/config.toml`.

    Returns:
        List of missing tool names (e.g. `["ripgrep"]`).
    """
    from oat_cli.model_config import is_warning_suppressed
...
def parse_args() -> argparse.Namespace:
    ...
    """
    from oat_cli.skills import setup_skills_parser
    from oat_cli.ui import (
        build_help_parent,
        show_help,
        show_list_help,
        show_reset_help,
    ...
    # `oat_sdk <subcommand> -h` shows context-specific help.
    ...
    parser = argparse.ArgumentParser(
        description=("OAT - AI Coding Assistant"),
    ...
    parser.add_argument(
        "--thread-id",
        dest="custom_thread_id",
        metavar="ID",
        help="Use custom thread ID for new session (OAT integration)",
    )
...
async def run_textual_cli_async(
    ...
    *,
    ...
    from oat_cli.app import run_textual_app
    from oat_cli.config import console, create_model, settings
    from oat_cli.model_config import ModelConfigError
    from oat_cli.sessions import get_checkpointer
...
def apply_stdin_pipe(args: argparse.Namespace) -> None:
    ...
    - If `non_interactive_message` is already set (`-n`), prepends the
        ...
        ```bash
        cat context.txt | oat_sdk -n "summarize this"
    ...
    - If `initial_prompt` is already set (`-m`, but not `-n`), prepends
        ...
        ```bash
        cat error.log | oat_sdk -m "explain this"
    ...
    - Otherwise, sets `non_interactive_message` to the piped text, causing
        ...
        ```bash
        echo "fix the typo in README.md" | oat_sdk
    ...
    """
    from oat_cli.config import console
...
def cli_main() -> None:
    ...
    # OAT_LANGSMITH_PROJECT while shell commands use the
    ...
    if len(sys.argv) == 2 and sys.argv[1] in {"-v", "--version"}:  # noqa: PLR2004  # argv length check for fast-path
        try:
            ...
            sdk_version = "unknown"
        print(f"oat-cli {__version__}\noat_sdk {sdk_version}")  # noqa: T201  # CLI version output
        sys.exit(0)

    # Check dependencies first
    check_cli_dependencies()

    from oat_cli.config import console, settings
    ...
    try:
        ...
        if args.default_model is not None:
            ...
            from oat_cli.config import detect_provider
            from oat_cli.model_config import ModelSpec
        ...
        if args.command == "help":
            ...
            from oat_cli.agent import list_agents

            list_agents()
        elif args.command == "reset":
            from oat_cli.agent import reset_agent

            reset_agent(args.agent, args.source_agent)
        elif args.command == "skills":
            from oat_cli.skills import execute_skills_command
            ...
            from oat_cli.ui import show_threads_help
            ...
            from oat_cli.non_interactive import run_non_interactive
            ...
            if args.resume_thread == "__MOST_RECENT__":
                ...
                if asyncio.run(thread_exists(args.resume_thread)):
                    ...
                    if similar:
                        ...
                        for tid in similar:
                            hint = Text("  oat_sdk -r ", style="cyan")
                            hint.append(str(tid), style="cyan")
                            console.print(hint)
                        console.print()

                    console.print(
                        "[dim]Use 'oat_sdk threads list' to see "
            ...
            if thread_id is None:
                if args.custom_thread_id:
                    # Use custom thread ID provided by OAT
                    thread_id = args.custom_thread_id
                else:
