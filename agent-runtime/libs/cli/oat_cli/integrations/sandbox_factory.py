...
from oat_cli.config import console, get_glyphs
...
if TYPE_CHECKING:
    ...
    from oat_cli.integrations.sandbox_provider import SandboxProvider
...
def _get_provider(provider_name: str) -> SandboxProvider:
    ...
    if provider_name == "daytona":
        from oat_cli.integrations.daytona import DaytonaProvider

        return DaytonaProvider()
    if provider_name == "langsmith":
        from oat_cli.integrations.langsmith import LangSmithProvider

        return LangSmithProvider()
    if provider_name == "modal":
        from oat_cli.integrations.modal import ModalProvider

        return ModalProvider()
    if provider_name == "runloop":
        from oat_cli.integrations.runloop import RunloopProvider
