...
If running oat_sdk from a virtual environment:
```bash
...
Replace `[YOUR_SKILLS_DIR]` with the absolute skills directory path from your system prompt (e.g., `~/.oat_sdk/agent/skills` or the full absolute path).
...
```bash
.venv/bin/python ~/.oat_sdk/agent/skills/arxiv-search/arxiv_search.py "deep learning drug discovery" --max-papers 5
```

Search for computational biology papers:
```bash
.venv/bin/python ~/.oat_sdk/agent/skills/arxiv-search/arxiv_search.py "protein folding prediction"
```

Search for bioinformatics methods:
```bash
.venv/bin/python ~/.oat_sdk/agent/skills/arxiv-search/arxiv_search.py "genome assembly algorithms"
```
...
If running oat_sdk from a virtual environment (recommended), use the venv's Python:
```bash
...
The package is not included in oat_sdk by default since it's skill-specific. Install it on-demand when first using this skill.
