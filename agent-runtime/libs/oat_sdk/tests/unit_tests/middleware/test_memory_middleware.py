...
def test_create_oat_agent_with_memory_and_filesystem_backend(tmp_path: Path) -> None:
    """Test end-to-end: create_oat_agent with memory parameter and FilesystemBackend."""
    # Create memory on filesystem
    backend = FilesystemBackend(root_dir=str(tmp_path), virtual_mode=False)
    memory_path = str(tmp_path / "user" / "AGENTS.md")
    memory_content = make_memory_content("OAT Agent Test", "- Use deep agents wisely")
...
def test_create_oat_agent_with_memory_missing_files(tmp_path: Path) -> None:
...
def test_create_oat_agent_with_memory_default_backend() -> None:
    """Test create_oat_agent with memory parameter using default backend (StateBackend).
    ...
    assert "/user/.oat_sdk/AGENTS.md" in checkpoint["channel_values"]["files"]
    assert "memory_contents" in checkpoint["channel_values"]
    assert "/user/.oat_sdk/AGENTS.md" in checkpoint["channel_values"]["memory_contents"]
