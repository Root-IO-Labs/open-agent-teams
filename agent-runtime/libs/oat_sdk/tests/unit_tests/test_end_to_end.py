"""End-to-end unit tests for oat_sdk with fake LLM models."""
...
from oat_sdk.backends import FilesystemBackend
from oat_sdk.backends.protocol import BackendProtocol
from oat_sdk.backends.state import StateBackend
from oat_sdk.backends.store import StoreBackend
from oat_sdk.backends.utils import TOOL_RESULT_TOKEN_LIMIT
from oat_sdk.graph import create_oat_agent
from oat_sdk.middleware.filesystem import NUM_CHARS_PER_TOKEN
from tests.utils import SampleMiddlewareWithTools, SampleMiddlewareWithToolsAndState, assert_all_oat_sdk_qualities
...
class TestOatSdkEndToEnd:
    """Test suite for end-to-end oat_sdk functionality with fake LLM."""

    def test_deep_agent_with_fake_llm_basic(self) -> None:
        """Test basic oat_sdk functionality with a fake LLM model.

        This test verifies that a oat_sdk can be created and invoked with
        ...
        agent = create_oat_agent(model=model)
    ...
    def test_deep_agent_with_fake_llm_with_tools(self) -> None:
        """Test oat_sdk with tools using a fake LLM model.

        This test verifies that a oat_sdk can handle tool calls correctly
        ...
        agent = create_oat_agent(model=model, tools=[sample_tool])
    ...
    def test_deep_agent_with_fake_llm_filesystem_tool(self) -> None:
        """Test oat_sdk with filesystem tools using a fake LLM model.

        This test verifies that a oat_sdk can use the built-in filesystem
    ...
    def test_deep_agent_with_fake_llm_multiple_tool_calls(self) -> None:
        """Test oat_sdk with multiple tool calls using a fake LLM model.

        This test verifies that a oat_sdk can handle multiple sequential
    ...
    def test_deep_agent_with_string_model_name(self) -> None:
        """Test that create_oat_agent handles string model names correctly.
        ...
        with patch("oat_sdk.graph.init_chat_model", return_value=fake_model):
            # This should not raise AttributeError: 'str' object has no attribute 'profile'
            agent = create_oat_agent(model="claude-sonnet-4-6", tools=[sample_tool])
    ...
    def test_deep_agent_truncate_lines(self, tmp_path: Path, backend_factory: Callable[[Path], BackendProtocol]) -> None:
        ...
        agent = create_oat_agent(model=model, backend=backend)
    ...
    def test_deep_agent_with_system_message(self) -> None:
        """Test that create_oat_agent accepts a SystemMessage for system_prompt."""
        model = FixedGenericFakeChatModel(
            messages=iter(
                [
        ...
        agent = create_oat_agent(model=model, system_prompt=system_msg, middleware=[capturing_middleware])
        assert_all_oat_sdk_qualities(agent)
        ...
        assert "You are an OAT Agent" in content

    def test_deep_agent_with_system_message_string_content(self) -> None:
        """Test that create_oat_agent accepts a SystemMessage with string content."""
        model = FixedGenericFakeChatModel(
            messages=iter(
                [
    ...
    def test_deep_agent_two_turns_no_initial_files(self) -> None:
        """Test oat_sdk with two conversation turns without specifying files on invoke.
    ...
    async def test_deep_agent_two_turns_no_initial_files_async(self) -> None:
        """Async version: Test oat_sdk with two conversation turns without specifying files.
    ...
    def test_read_large_single_line_file_returns_reasonable_size(self) -> None:
        ...
        agent = create_oat_agent(model=fake_model)
...
class TestOatSdkStructure:
    ...
    def test_deep_agent_with_tool(self) -> None:
        ...
        agent = create_oat_agent(tools=[sample_tool])
        assert_all_oat_sdk_qualities(agent)
    ...
    def test_deep_agent_with_middleware_with_tool(self) -> None:
        ...
        agent = create_oat_agent(middleware=[SampleMiddlewareWithTools()])
        assert_all_oat_sdk_qualities(agent)
        assert "sample_tool" in agent.nodes["tools"].bound._tools_by_name
    ...
    def test_deep_agent_with_middleware_with_tool_and_state(self) -> None:
        ...
        agent = create_oat_agent(middleware=[SampleMiddlewareWithToolsAndState()])
        assert_all_oat_sdk_qualities(agent)
        assert "sample_tool" in agent.nodes["tools"].bound._tools_by_name
