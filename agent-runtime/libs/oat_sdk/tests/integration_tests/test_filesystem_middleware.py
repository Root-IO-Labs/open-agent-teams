...
from oat_sdk.backends import CompositeBackend, StateBackend, StoreBackend
from oat_sdk.backends.protocol import ExecuteResponse, SandboxBackendProtocol
from oat_sdk.graph import create_oat_agent
from oat_sdk.middleware.filesystem import (
    FileData,
...
class TestFilesystem:
    ...
    def test_longterm_memory_multiple_tools_oat_sdk(self):
        checkpointer = MemorySaver()
        store = InMemoryStore()
        ...
        agent = create_oat_agent(backend=backend, checkpointer=checkpointer, store=store)
        assert_longterm_mem_tools(agent, store)

    def test_shortterm_memory_multiple_tools_oat_sdk(self):
        checkpointer = MemorySaver()
        store = InMemoryStore()
        agent = create_oat_agent(backend=StateBackend, checkpointer=checkpointer, store=store)
