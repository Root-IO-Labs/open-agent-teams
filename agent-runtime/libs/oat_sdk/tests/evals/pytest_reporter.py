...
from oat_sdk.graph import get_default_model
...
def pytest_addoption(parser: pytest.Parser) -> None:
    parser.addoption(
        "--evals-report-file",
        action="store",
        default=os.environ.get("OAT_EVALS_REPORT_FILE"),
        help=("Write a JSON eval report to this path. If omitted, no JSON report is written. Can also be set via OAT_EVALS_REPORT_FILE."),
    )
...
def pytest_sessionfinish(session: pytest.Session, exitstatus: int) -> None:
    ...
    if terminal_reporter is not None:
        terminal_reporter.write_sep("=", "oat_sdk evals summary")
