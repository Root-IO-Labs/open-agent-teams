# Development guidelines for the OAT agent runtime

This document provides context to understand the OAT Python agent runtime and assist with development.
...
agent-runtime/
├── libs/
│   ├── oat_sdk/     # Core SDK (create_oat_agent, middleware, backends)
│   └── cli/         # OAT CLI tool (oat_cli)
└── services/
    └── agent-builder/  # Agent Builder (interactive agent type creator)
```
...
### Agent CLI — `oat_sdk-cli` (`libs/cli/`)

`oat_sdk-cli` uses [Textual](https://textual.textualize.io/) for its terminal UI framework.
...
The CLI pins an exact `oat_sdk==X.Y.Z` version in `libs/cli/pyproject.toml`. When developing CLI features that depend on new SDK functionality, bump this pin as part of the same PR. A CI check verifies the pin matches the current SDK version at release time (unless bypassed with `dangerous-skip-sdk-pin-check`).

**Startup performance:**

The CLI must stay fast to launch. Never import heavy packages (e.g., `oat_sdk`, LangChain, LangGraph) at module level or in the argument-parsing path. These imports pull in large dependency trees and add seconds to every invocation, including trivial commands like `oat-agent --version`.
...
The `oat-agent --help` screen is hand-maintained in `ui.show_help()`, separate from the argparse definitions in `main.parse_args()`. When adding a new CLI flag, update **both** files. A drift-detection test (`test_args.TestHelpScreenDrift`) fails if a flag is registered in argparse but missing from the help screen.
...
- **Upstream SDK docs:** https://docs.langchain.com/oss/python/oat_sdk/overview — the `oat_sdk` SDK is an upstream dependency; prefer local source and file search tools over the hosted docs.
