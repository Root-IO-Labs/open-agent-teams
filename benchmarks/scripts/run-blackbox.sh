...
# BENCH_DIR is the parent of scripts/, i.e. the user-facing benchmarks/
# directory. The default results path needs to live under benchmarks/
# (not benchmarks/scripts/) so results co-locate with the existing
# benchmarks/results/ layout shared by run.sh and acceptance-test.sh.
BENCH_DIR="$(cd "${SCRIPT_DIR}/.." && pwd)"
GITHUB_OWNER="${GITHUB_OWNER:-$(gh api /user --jq '.login' 2>/dev/null || echo 'Root-IO-Labs')}"

usage() {
    cat <<'EOF'
Usage: ./benchmarks/scripts/run-blackbox.sh --test <path> --repo <name> --output <path> [options]
...
if [[ -z "$OUTPUT" ]]; then
    OUTPUT="${BENCH_DIR}/results/blackbox-acceptance.json"
fi
