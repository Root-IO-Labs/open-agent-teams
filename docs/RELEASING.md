...
```bash
make release-check     # validates .goreleaser.yaml syntax
...
GoReleaser auto-generates the release notes from commit messages between the previous tag and the current one, excluding commits with `docs:`, `test:`, `chore:`, or `Merge` prefixes (see `.goreleaser.yaml`).
