...
| `OAT_BRIDGE_CANCEL` | no | On agent interrupt (Stop / redirect), propagate MCP `notifications/cancelled` to the bridge and suppress the response for the aborted in-flight tool call (avoids the strict-client transport drop in python-sdk#2480). Default on; set falsey to disable. |
| `OAT_SCREENSHOT_BYTE_CAP` | no | Bridge-side base64 byte cap (default ~4.5 MB) for a single screenshot image block; an oversized block is dropped and replaced with a text explanation so a request doesn't 400 on "image exceeds maximum size". The extension also pre-clamps captures to ≤2000px/side under this budget. |
