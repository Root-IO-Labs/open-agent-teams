...
| First browser tool after burger / Manage-tab Restart / `oat assistant restart` fails with `EXTENSION_NOT_CONNECTED` | Per-agent tool socket not re-bound yet (registry lag), or MCP bridge double-boot after `--fresh` (bind succeeds then drops without a second `agent_started`) | Wait for auto-rebind (jittered NM retries + a 30s post-ready stability watch that re-arms if the socket drops again; Manage Restart posts the same `refresh_bridge_registry` kick as burger Restart) | Prefer waiting for the panel's auto-reconnect; **reload the OAT extension only if the reconnect budget fails** and the panel shows the reload CTA |
| First browser tool after a Manage settings model pick + Send fails with `EXTENSION_NOT_CONNECTED` | Deferred model-switch restart (session-preserving) just re-spawned the agent | Same reconnect path as Manage Restart / `agent_started` retries | Wait for auto-rebind; do not use `--fresh` on the model-switch path |
