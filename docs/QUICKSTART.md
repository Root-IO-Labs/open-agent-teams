...
If you already have Go 1.25+, Python 3.11+, `uv`, `git`, and `gh` installed and at least one provider API key exported (`ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, or any other supported provider — see the [Prerequisites](#prerequisites) table below):
