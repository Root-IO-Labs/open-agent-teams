...
```json
{
  ...
  "model": "claude-sonnet-4-6",       // LLM model preference (agent-level); empty = resolve from repo/default at spawn. Side-panel display uses resolved_model for the running id — never invent a literal "default" string when model is empty.
  "display_name": "",                 // Cosmetic Manage-tab / chat-picker alias; empty = show agent slug. Not a path key.
  ...
  "context_window_tokens": 0,        // Current context-window occupancy; drives the live capacity %
  ...
  "resolved_model": "",               // Spawn-time resolved model — the literal `-M` value handed to the agent runtime, frozen at spawn so later daemon-default / set-model preference changes can't lie about what's actually running until the next (re)spawn. Lifecycle frames expose this as `model` (running) vs `configured_model` (preference).
  ...
  "last_error": ""                    // Most-recent lifecycle event that produced an error/halt. Set to "stopped by user" by handleStopAgent so the side panel + recovery audit can see why PID is zero without grepping daemon.log.
}
```
