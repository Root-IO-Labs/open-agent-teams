...
```json
{
  ...
  "rejection_count": 0,               // Number of times verification has rejected this worker (workers only); used for rejection-cap escalation
  "base_sha": "",                     // Pinned remote default-branch SHA for the verifier diff (workers only; set at request-review)
  "routing_source": "",               // Tag identifying which routing rule chose this agent's model: operator-explicit | repo-default | router-auto | restart-fallback | unknown
  "routing_decision_reason": "",      // Human-readable explanation from the router (e.g. "complexity=complex floor=9 chose haiku")
  "routing_candidates": [],           // Ranked candidate model IDs at decision time, first = the pick (used for counterfactual replay)
  "routing_allowlist": [],            // Snapshot of repo's AllowedWorkerModels at decision time
  "prompt_system_hash": "",           // sha256 of the system prompt at spawn — feeds OutcomeRecord.Prompt
  "prompt_system_tokens": 0,          // Estimated token count of the system prompt
  "prompt_user_hash": "",             // sha256 of the user task message at spawn
  "prompt_user_tokens": 0,            // Estimated token count of the user task message
