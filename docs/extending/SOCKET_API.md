...
| `stream_agent_lifecycle` | **(streaming verb.)** Pushes agent add/start/stop/remove events so the side-panel Status-tab cards refresh reactively without polling. On subscribe the daemon replays one `snapshot` frame per existing agent across all repos, then streams subsequent lifecycle events. Frame shape: `{kind, repo, agent, agent_type, pid, model, last_error, ts}` with `kind ∈ {snapshot, agent_added, agent_started, agent_stopped, agent_removed}`. NOT scoped by `AgentType` — the bridge needs every agent so the chat-tab picker can drive its dropdown from the same feed. Filtering happens client-side. Best-effort: a stuck subscriber's frames are dropped silently after the per-subscription buffer fills; other subscribers continue to receive. | (no args) |
