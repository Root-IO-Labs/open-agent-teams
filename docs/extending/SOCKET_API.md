...
| `stream_assistant_turns` | **(streaming verb.)** Per-turn structured event stream for chat-capable agents: turn-start, tool-call, tool-result, assistant-message, turn-end. The oat-browser-agent bridge's `AssistantTurnMultiplexer` (Part 8 Commit 8.4) opens one subscription per chat-capable agent in its lifecycle inventory and fans the events out as side-panel chat bubbles tagged with the originating `(repo, agent)` pair. Distinct from `stream_agent_output` because the bridge needs the structured turn semantics (e.g. to render tool-call panels) — the raw PTY bytes don't carry that information cleanly. **Part 8 Commit 8.2 lifted the type gate** from `AgentTypeAssistant`-only to any chat-capable agent (the daemon helper `usesBrowserBridge` enforces this: `AgentTypeAssistant + AgentTypeBrowser` pass; supervisor/worker/merge-queue/pr-shepherd/etc. are rejected with `"stream_assistant_turns is restricted to chat-capable agents (assistant + browser)"`). | `session` (string), `agent` (string) |
