...
- **Harness liveness (primary)**: early name-only `TOOL` + timer-based `[OAT_GENERATING]` heartbeats keep the panel showing last-tool + elapsed during long arg generation (including buffered bodies with no further stream chunks); mid-turn side-panel Send is interrupt-then-route (stuck wording only chooses the diagnose prefix). Manage-tab Restart uses the same bridge reconnect kick as burger Restart. Recovery below is the backstop, not the main UX.
- **Self-healing turn ladder (backstop)**: when an assistant turn ends with no visible reply, the daemon may inject one bounded, code-only recovery re-prompt (`internal/daemon/assistant_recovery.go`; shared budget `OAT_ASSISTANT_RECOVERY_MAX`, default 1; no-op under `OAT_TEST_MODE`). Triggers: (1) last tool errored on a fail-closed allowlist, or (2) **incomplete silent stop** — the turn ran tools successfully then ended without a chat bubble (that consume is refunded once if the model then makes more tool progress; a further silent allowlisted error may chain one more inject). Separately, a **plan-stale nudge** may remind the model to call `write_todos` after progress tools when the Plan card is unfinished (does not share the recovery budget). User Stop / mid-turn Send latches the turn so recovery is suppressed. Pure status pings keep the budget; new work resets it. The turn boundary comes from the `[OAT_TURN_END]` sentinel the runtime writes to `OAT_TOOL_LOG`, surfaced to the panel as a `turn_end` agent-activity frame. See the root `AGENTS.md` harness-liveness + self-healing notes for the full description.
...
| **Daemon: output-headroom reservation** | `internal/daemon/context_capacity.go` | With `OAT_CONTEXT_RESERVE_OUTPUT` (default ON, tri-state fail-safe ON), the capacity **tiers** compute against the `(window − output)` budget so the agent doesn't overflow while emitting a long reply; the passive **ring** still shows the full window. |
| **Agent-runtime: per-call output-token cap** | `agent-runtime` summarization middleware | Defense-in-depth per-call cap on output tokens so a single turn can't blow the budget with a runaway generation. Disable for a clean rollback via `OAT_DISABLE_OUTPUT_TOKEN_CAP` (cap ON by default). |
...
| **Bridge: screenshot byte cap** | `oat-browser-agent/bridge` | `OAT_SCREENSHOT_BYTE_CAP` (~4.5 MB) drops-and-explains an oversized screenshot image block; the extension pre-clamps captures to ≤2000px/side under the same budget so a big page can't produce a request-killing oversized image. |
...
**Retry-spiral backstop (loop-breaker).** Separate from context overflow: an agent-runtime middleware (`agent-runtime` `loop_breaker.py`, wired in `graph.py`) detects **N consecutive same-tool + same-error-code** failures with no intervening progress (e.g. `REF_STALE`×3, a repeated auth/session-expiry) and **stops and reports** — emitting a clear message of what it tried and the error — instead of grinding forever. The counter resets on any successful tool call or DOM/URL change, so benign polling (`browser_wait_for`) doesn't trip it. Threshold is `OAT_LOOP_BREAKER_MAX` (default 3). In side-panel chat mode it stops and asks the user; for task-bound agents with no operator it escalates/completes-with-failure. Defense-in-depth behind the `browser.md` prompt rule to change strategy after repeated identical failures.

**Browser/assistant model-suitability guard (advisory).** At `oat agent add browser-agent --model <id>` and `oat agent set-model` time (for Browser/Assistant targets), the CLI prints an advisory WARN when the model's onboarded profile is a poor fit for interactive browsing — low `shell_recovery` or very high `basic_inference_ms` (`warnIfPoorBrowsingModel` in `internal/cli/cli.go`). Advisory only (never a hard block — bring-your-own-model still works); thresholds are tunable via `OAT_BROWSER_MODEL_MIN_SHELL_RECOVERY` / `OAT_BROWSER_MODEL_MAX_INFERENCE_MS`.
