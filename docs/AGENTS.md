...
- **Self-healing turn ladder**: when an assistant turn ends with no visible reply, the daemon may inject one bounded, code-only recovery re-prompt (`internal/daemon/assistant_recovery.go`; shared budget `OAT_ASSISTANT_RECOVERY_MAX`, default 1; no-op under `OAT_TEST_MODE`). Triggers: (1) last tool errored on a fail-closed allowlist, or (2) **incomplete silent stop** — the turn ran tools successfully then ended without a chat bubble (that consume is refunded once if the model then makes more tool progress). User Stop / interrupt latches the turn so recovery is suppressed. Pure status pings keep the budget; new work resets it. The turn boundary comes from the `[OAT_TURN_END]` sentinel the runtime writes to `OAT_TOOL_LOG`, surfaced to the panel as a `turn_end` agent-activity frame. See the root `AGENTS.md` "Self-healing turn ladder" note for the full three-layer description.
