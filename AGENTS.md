...
**Self-healing turn ladder (assistant "went silent after a tool error" fix):** a three-layer mechanism so a stuck assistant recovers on its own instead of leaving the user to prod it. **Layer 1 (bridge):** transient failures on SAFE/read-only tools are silently retried with jittered backoff before the model sees them (`oat-browser-agent/bridge/src/silent-retry.ts`; mutating tools excluded). **Layer 2 (daemon):** when a turn ends with its last tool call in error and no visible reply, the daemon injects exactly one bounded, code-only `[OAT-system]` re-prompt from a curated fail-closed allowlist (`internal/daemon/assistant_recovery.go`; budget `OAT_ASSISTANT_RECOVERY_MAX`, default 1). The allowlist covers genuinely model-fixable bridge codes only — tab-addressing (`TAB_CLOSED`, `NO_ACTIVE_TAB`, `DEBUGGER_ATTACH_FAILED`, …), stale/failed element interactions (`STALE_REF`, `ELEMENT_NOT_FOUND`, `CLICK_FAILED`, `TYPE_FAILED`, …), nav/wait (`NAVIGATION_FAILED`, `WAIT_TIMEOUT`), bad args (`INVALID_PARAMS`, …), transient captures (`SCREENSHOT_FAILED`, …); security/policy/user-recoverable/generic codes are surfaced, never auto-recovered. **Layer 3 (panel):** non-recoverable errors surface as a plain-language outcome note. The turn boundary is detected via the `[OAT_TURN_END]` sentinel the runtime writes to `OAT_TOOL_LOG` (`emit_turn_end`), parsed into the `turn_end` agent-activity frame carrying `{hadError, code, retryable, visibleReply, recovering}`.
