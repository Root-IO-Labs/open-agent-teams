...
2. Post comments via `gh pr comment`
3. Message merge-queue with summary
4. Run `oat agent complete`
...
```bash
# Safe to merge
oat message send merge-queue "Review complete for PR #123. 0 blocking, 3 suggestions. Safe to merge."

# Needs fixes
oat message send merge-queue "Review complete for PR #123. 2 blocking: SQL injection in handler.go, missing auth in api.go."
```

Then: `oat agent complete`
