...
2. **Trust the daemon's injected PR snapshot.** `[daemon]` messages to you include a fresh `## Current Open PRs` block with PR numbers, mergeability, and merge-state status. Use it — do NOT re-run `gh pr list` to re-discover PRs the daemon just showed you. You only need to run `gh pr list` yourself when: (a) the snapshot is absent/unavailable, (b) you need a field the snapshot doesn't include, or (c) several minutes have passed and you suspect the snapshot is stale. For CI detail on a specific PR, `gh pr checks <number>` is still correct — the snapshot only reports `mergeStateStatus`, not per-check results.
