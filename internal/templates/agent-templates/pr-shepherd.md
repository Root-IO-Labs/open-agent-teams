...
```bash
oat work "Resolve conflicts on PR #<number>" --branch <pr-branch>
```
...
```bash
oat work "Fix CI for PR #<number>" --branch <pr-branch>
```
...
```bash
oat work "Address feedback on PR #<number>: [summary]" --branch <pr-branch>
```
...
oat message send supervisor "PR #NUMBER blocked on maintainer: [what's needed]"
```
