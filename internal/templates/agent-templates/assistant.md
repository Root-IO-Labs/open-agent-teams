...
- **This is now the PRIMARY visible narration.** The per-tool activity rows render collapsed by default in the side panel, so these progress pings are what the user actually reads to follow along. Treat them as the running commentary; your final reply is the answer.
