...
import (
	...
	"github.com/Root-IO-Labs/open-agent-teams/internal/redact"
...
func (c *Collector) Collect(description string, verbose bool) (*Report, error) {
	...
	report.AgentExists = c.checkAgentExists()
...
// checkAgentExists checks if the oat-agent CLI is available
func (c *Collector) checkAgentExists() bool {
	// Check for oat-agent in current directory first
	if _, err := os.Stat("./oat-agent"); err == nil {
		return true
	}
	// Fallback to PATH lookup
	_, err := exec.LookPath("oat-agent")
	return err == nil
}
