...
func TestHasUncommittedChanges_OatDirExcluded(t *testing.T) {
	repoPath, cleanup := createTestRepo(t)
	defer cleanup()

	manager := NewManager(repoPath)

	wtPath := filepath.Join(repoPath, "wt-oat-filter")
	if err := manager.CreateNewBranch(wtPath, "oat-filter-branch", "main"); err != nil {
		t.Fatalf("Failed to create worktree: %v", err)
	}

	// Create .oat/.gitignore (what OAT writes at runtime)
	oatDir := filepath.Join(wtPath, ".oat")
	if err := os.MkdirAll(oatDir, 0755); err != nil {
		t.Fatalf("Failed to create .oat dir: %v", err)
	}
	if err := os.WriteFile(filepath.Join(oatDir, ".gitignore"), []byte("*\n"), 0644); err != nil {
		t.Fatalf("Failed to write .oat/.gitignore: %v", err)
	}
	...
	if hasChanges {
		t.Error(".oat/ files alone should not trigger HasUncommittedChanges")
	}

	// A real file alongside .oat/ SHOULD trigger uncommitted changes
	if err := os.WriteFile(filepath.Join(wtPath, "real-change.txt"), []byte("hello"), 0644); err != nil {
	...
	if !hasChanges {
		t.Error("Real file changes should trigger HasUncommittedChanges even with .oat/ present")
	}
}
...
func TestFindMergedUpstreamBranches(t *testing.T) {
	...
	t.Run("respects prefix filter", func(t *testing.T) {
		...
		createBranch(t, repoPath, "oat/test")
...
func TestListBranchesWithPrefix(t *testing.T) {
	t.Run("lists matching branches", func(t *testing.T) {
		...
		createBranch(t, repoPath, "oat/agent-1")
...
func TestFindOrphanedBranches(t *testing.T) {
	...
	t.Run("respects prefix filter", func(t *testing.T) {
		...
		createBranch(t, repoPath, "oat/orphan")
...
func TestCleanupOrphanedWithDetails(t *testing.T) {
	...
	t.Run("handles non-existent directory", func(t *testing.T) {
		...
		result, err := CleanupOrphanedWithDetails("/nonexistent/directory", manager, nil)
		if err != nil {
			t.Fatalf("Should not error for non-existent directory: %v", err)
		}
...
func TestCleanupOrphanedWithProtectedPaths(t *testing.T) {
	t.Run("protected paths survive cleanup", func(t *testing.T) {
		repoPath, cleanup := createTestRepo(t)
		defer cleanup()
		...
		protectedDir := filepath.Join(wtRootDir, "active-worker")
		orphanedDir := filepath.Join(wtRootDir, "orphaned-worker")
		...
		protectedPaths := map[string]bool{protectedDir: true}
		result, err := CleanupOrphanedWithDetails(wtRootDir, manager, protectedPaths)
		if err != nil {
			t.Fatalf("CleanupOrphanedWithDetails failed: %v", err)
		}

		if len(result.Removed) != 1 {
			t.Errorf("Expected 1 removed (orphaned only), got %d: %v", len(result.Removed), result.Removed)
		}
		if len(result.Removed) == 1 && result.Removed[0] != orphanedDir {
			t.Errorf("Removed wrong directory: got %s, want %s", result.Removed[0], orphanedDir)
		}

		if _, err := os.Stat(protectedDir); os.IsNotExist(err) {
			t.Error("Protected directory should NOT have been deleted")
		}

		if reason, ok := result.Errors[protectedDir]; !ok {
			t.Error("Protected path should appear in errors with skip reason")
		} else if reason != "skipped: directory belongs to active agent (git tracking out of sync)" {
			t.Errorf("Unexpected skip reason: %s", reason)
		}
	})

	t.Run("nil protectedPaths deletes everything", func(t *testing.T) {
		repoPath, cleanup := createTestRepo(t)
		defer cleanup()
		...
		dirA := filepath.Join(wtRootDir, "dir-a")
		dirB := filepath.Join(wtRootDir, "dir-b")
		...
		if len(result.Removed) != 2 {
			t.Errorf("Expected 2 removed with nil protectedPaths, got %d", len(result.Removed))
		}
	})
}
