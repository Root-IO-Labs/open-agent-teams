...
const (
	...
	StatusFailed    Status = "failed" // delivery permanently failed (e.g. agent adopted/dead)
)
...
func (m *Manager) List(repoName, agentName string) ([]*Message, error) {
	...
	// Sort chronologically so messages are delivered/displayed in order
...
// PurgePending removes all pending (undelivered) messages for an agent.
// Used when an agent completes to prevent stale messages from being processed.
func (m *Manager) PurgePending(repoName, agentName string) (int, error) {
	messages, err := m.List(repoName, agentName)
	if err != nil {
		return 0, err
	}
