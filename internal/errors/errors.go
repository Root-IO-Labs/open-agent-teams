...
func AgentNotFound(agentType, name, repo string) *CLIError {
	return &CLIError{
		...
		Suggestion: fmt.Sprintf("oat worker list --repo %s", repo),
	}
}
...
// AgentBinaryNotFound creates an error for when oat-agent binary is not found
func AgentBinaryNotFound(cause error) *CLIError {
	return &CLIError{
		Category:   CategoryConfig,
		Message:    "oat-agent binary not found in PATH or current directory",
		Cause:      cause,
		Suggestion: "install or build oat-agent and ensure it is in PATH",
	}
}
...
func NotInAgentContext() *CLIError {
	return &CLIError{
		Category:   CategoryConfig,
		Message:    "not in a oat agent directory",
		Suggestion: "run this command from within an agent session or use --repo flag",
	}
}
...
func NoRepositoriesFound() *CLIError {
	return &CLIError{
		...
		Suggestion: "oat repo init <github-url>",
	}
}
...
func NoWorkersFound(repo string) *CLIError {
	return &CLIError{
		...
		Suggestion: fmt.Sprintf("oat worker create \"<task>\" --repo %s", repo),
	}
}
...
func NoWorkspacesFound(repo string) *CLIError {
	return &CLIError{
		...
		Suggestion: fmt.Sprintf("oat workspace add <name> --repo %s", repo),
	}
}
