...
func TestFormat_CLIError(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			err:      New(CategoryConnection, "daemon offline").WithSuggestion("oat daemon start"),
			contains: []string{"daemon offline", "Try:", "oat daemon start"},
		},
	}
...
func TestDaemonNotRunning(t *testing.T) {
	...
	if !strings.Contains(formatted, "oat start") {
...
func TestNotInRepo(t *testing.T) {
	...
	if !strings.Contains(formatted, "oat repo init") {
...
func TestAgentNotFound(t *testing.T) {
	...
	if !strings.Contains(formatted, "oat worker list") {
...
func TestAgentBinaryNotFound(t *testing.T) {
	err := AgentBinaryNotFound(errors.New("not found"))
	formatted := Format(err)

	if !strings.Contains(formatted, "oat-agent") {
		t.Errorf("expected oat-agent mention, got: %s", formatted)
	}
...
func TestWorktreeCreationFailed_SpecificSuggestions(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			wantContains: []string{"work/nice-owl", "oat cleanup", "git branch -D work/nice-owl"},
		},
		{
			name:         "generic branch already exists",
			causeMsg:     "branch already exists",
			wantContains: []string{"oat cleanup", "previous run"},
		},
		{
			name:         "worktree path exists",
			causeMsg:     "path already exists",
			wantContains: []string{"oat cleanup", "worktree directory"},
		},
		{
			name:         "is a worktree error",
			causeMsg:     "is a worktree",
			wantContains: []string{"oat cleanup"},
		},
		{
...
func TestNotInAgentContext(t *testing.T) {
	...
	if !strings.Contains(formatted, "agent session") {
		t.Errorf("expected 'agent session' in suggestion, got: %s", formatted)
	}
}
...
func TestNoWorkersFound(t *testing.T) {
	...
	if !strings.Contains(formatted, "oat worker create") {
...
func TestNoWorkspacesFound(t *testing.T) {
	...
	if !strings.Contains(formatted, "oat workspace add") {
...
func TestWorkspaceNotFound(t *testing.T) {
	...
	if !strings.Contains(formatted, "oat workspace list") {
