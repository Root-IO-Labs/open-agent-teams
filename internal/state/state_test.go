...
func TestAddAgentNilAgentsMap(t *testing.T) {
	tmpDir := t.TempDir()
	statePath := filepath.Join(tmpDir, "state.json")
	...
	repo := &Repository{
		GithubURL:   "https://github.com/test/repo",
		SessionName: "oat-test-repo",
		Agents:      nil, // simulate deserialized repo with missing agents key
	}
	...
	if err := s.AddAgent("test-repo", "supervisor", agent); err != nil {
		t.Fatalf("AddAgent() should not panic or fail with nil Agents map: %v", err)
	}

	got, exists := s.GetAgent("test-repo", "supervisor")
	if !exists {
		t.Fatal("agent not found after AddAgent with nil map")
	}
	if got.Type != AgentTypeSupervisor {
		t.Errorf("agent type = %v, want %v", got.Type, AgentTypeSupervisor)
	}
}
...
func TestModifyAgent(t *testing.T) {
	tmpDir := t.TempDir()
	statePath := filepath.Join(tmpDir, "state.json")
	...
	if err := s.ModifyAgent("test-repo", "worker", func(a *Agent) {
		a.NudgeCount = 3
		a.LastWakeReason = "test nudge"
	}); err != nil {
		t.Fatalf("ModifyAgent() failed: %v", err)
	}

	updated, exists := s.GetAgent("test-repo", "worker")
	if !exists {
		t.Fatal("Agent not found after ModifyAgent")
	}
	if updated.NudgeCount != 3 {
		t.Errorf("NudgeCount = %d, want 3", updated.NudgeCount)
	}
	if updated.LastWakeReason != "test nudge" {
		t.Errorf("LastWakeReason = %q, want %q", updated.LastWakeReason, "test nudge")
	}
	if !updated.WaitingForPR {
		t.Error("WaitingForPR was clobbered by ModifyAgent; should remain true")
	}
}

func TestModifyAgentPreservesConcurrentChanges(t *testing.T) {
	tmpDir := t.TempDir()
	statePath := filepath.Join(tmpDir, "state.json")
	...
	// Simulate handleAgentWaiting: set dormancy
	if err := s.ModifyAgent("test-repo", "worker", func(a *Agent) {
		a.WaitingForPR = true
		a.NudgeCount = 0
	}); err != nil {
		t.Fatalf("ModifyAgent (dormancy) failed: %v", err)
	}

	// Simulate nudge loop writing back nudge fields only.
	// With the old UpdateAgent pattern, this would clobber WaitingForPR.
	// With ModifyAgent, WaitingForPR should be preserved.
	if err := s.ModifyAgent("test-repo", "worker", func(a *Agent) {
		if a.WaitingForPR {
			return
		}
		...
		t.Fatalf("ModifyAgent (nudge) failed: %v", err)
	}

	result, exists := s.GetAgent("test-repo", "worker")
	if !exists {
		t.Fatal("Agent not found")
	}
	if !result.WaitingForPR {
		t.Error("WaitingForPR should remain true after nudge-style ModifyAgent")
	}
	if result.NudgeCount != 0 {
		t.Errorf("NudgeCount = %d, want 0 (nudge should have been skipped due to WaitingForPR guard)", result.NudgeCount)
	}
}

func TestModifyAgentNonExistent(t *testing.T) {
	tmpDir := t.TempDir()
	statePath := filepath.Join(tmpDir, "state.json")

	s := New(statePath)

	if err := s.ModifyAgent("no-repo", "no-agent", func(a *Agent) {
		a.NudgeCount = 1
	}); err == nil {
		t.Error("ModifyAgent() should fail for nonexistent repo")
	}
	...
	if err := s.ModifyAgent("test-repo", "no-agent", func(a *Agent) {
		a.NudgeCount = 1
	}); err == nil {
		t.Error("ModifyAgent() should fail for nonexistent agent")
	}
}

func TestStateAgentReadyForCleanupAtAndIssueRoundTrip(t *testing.T) {
	tmpDir := t.TempDir()
	statePath := filepath.Join(tmpDir, "state.json")
	...
	got, exists := loaded.GetAgent("test-repo", "worker")
	if !exists {
		t.Fatal("Agent not found after load")
	}
	if got.ReadyForCleanupAt.IsZero() {
		t.Error("ReadyForCleanupAt did not persist")
	}
	if got.IssueNumber != "42" {
		t.Errorf("IssueNumber = %q, want %q", got.IssueNumber, "42")
	}
	if got.IssueURL != "https://github.com/owner/repo/issues/42" {
		t.Errorf("IssueURL = %q, want %q", got.IssueURL, "https://github.com/owner/repo/issues/42")
	}
}
...
func TestSaveCleansUpTempFiles(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		repo := &Repository{
			...
			SessionName: fmt.Sprintf("oat-test%d", i),
			Agents:      make(map[string]Agent),
		}
...
// ---------------------------------------------------------------------------
// Verification agent type and fields
// ---------------------------------------------------------------------------

func TestVerificationTypeIsNotPersistent(t *testing.T) {
	if AgentTypeVerification.IsPersistent() {
		t.Error("AgentTypeVerification.IsPersistent() = true, want false")
	}
}

func TestVerificationFieldsJSONRoundTrip(t *testing.T) {
	tmpDir := t.TempDir()
	statePath := filepath.Join(tmpDir, "state.json")

	s := New(statePath)
	_ = s.AddRepo("test-repo", &Repository{
		GithubURL:   "https://github.com/test/repo",
		SessionName: "oat-test-repo",
		Agents:      make(map[string]Agent),
	})
	...
	agent := Agent{
		...
		VerifiedCommitSHA:  "abc123def456",
		VerificationAt:     now,
		VerificationReason: "All checks passed",
	}

	if err := s.AddAgent("test-repo", "clever-fox", agent); err != nil {
		t.Fatalf("AddAgent() failed: %v", err)
	}
	...
	got, exists := loaded.GetAgent("test-repo", "clever-fox")
	if !exists {
		t.Fatal("Agent not found after load")
	}

	if got.VerificationAgent != "verify-clever-fox" {
		t.Errorf("VerificationAgent = %q, want %q", got.VerificationAgent, "verify-clever-fox")
	}
	if got.VerificationStatus != "approved" {
		t.Errorf("VerificationStatus = %q, want %q", got.VerificationStatus, "approved")
	}
	if got.VerifiedCommitSHA != "abc123def456" {
		t.Errorf("VerifiedCommitSHA = %q, want %q", got.VerifiedCommitSHA, "abc123def456")
	}
	if got.VerificationAt.IsZero() {
		t.Error("VerificationAt is zero after round-trip")
	}
	if got.VerificationReason != "All checks passed" {
		t.Errorf("VerificationReason = %q, want %q", got.VerificationReason, "All checks passed")
	}
}

func TestVerificationFieldsMissingInOldState(t *testing.T) {
	// Simulate loading a state file from before verification fields existed
	tmpDir := t.TempDir()
	statePath := filepath.Join(tmpDir, "state.json")
	...
	loaded, err := Load(statePath)
	if err != nil {
		t.Fatalf("Load() failed on old state: %v", err)
	}

	agent, exists := loaded.GetAgent("test-repo", "old-worker")
	if !exists {
		t.Fatal("Agent not found in old state")
	}

	// Verification fields should be zero values
	if agent.VerificationAgent != "" {
		t.Errorf("VerificationAgent = %q, want empty", agent.VerificationAgent)
	}
	if agent.VerificationStatus != "" {
		t.Errorf("VerificationStatus = %q, want empty", agent.VerificationStatus)
	}
	...
	if agent.Task != "Fix the bug" {
		t.Errorf("Task = %q, want %q", agent.Task, "Fix the bug")
	}
	if !agent.WaitingForPR {
		t.Error("WaitingForPR = false, want true")
	}
	if agent.PRNumber != 99 {
		t.Errorf("PRNumber = %d, want 99", agent.PRNumber)
	}
}

func TestModifyAgentVerificationDoesNotClobberOtherFields(t *testing.T) {
	tmpDir := t.TempDir()
	statePath := filepath.Join(tmpDir, "state.json")
	...
	// Modify only verification fields
	err := s.ModifyAgent("test-repo", "clever-fox", func(a *Agent) {
		a.VerificationAgent = "verify-clever-fox"
		a.VerificationStatus = "pending"
		a.VerifiedCommitSHA = ""
		a.VerificationReason = ""
		a.VerificationAt = time.Time{}
		a.LastBranchSHA = "fresh-sha-from-review"
	})
	if err != nil {
		t.Fatalf("ModifyAgent() failed: %v", err)
	}

	got, _ := s.GetAgent("test-repo", "clever-fox")

	// Verification fields should be set
	if got.VerificationAgent != "verify-clever-fox" {
		t.Errorf("VerificationAgent = %q, want %q", got.VerificationAgent, "verify-clever-fox")
	}
	if got.VerificationStatus != "pending" {
		t.Errorf("VerificationStatus = %q, want %q", got.VerificationStatus, "pending")
	}

	// Other fields should be untouched
	if got.Task != "Implement feature X" {
		t.Errorf("Task clobbered: got %q, want %q", got.Task, "Implement feature X")
	}
	if !got.WaitingForPR {
		t.Error("WaitingForPR clobbered: got false, want true")
	}
	if got.PRNumber != 42 {
		t.Errorf("PRNumber clobbered: got %d, want 42", got.PRNumber)
	}
	if got.Model != "claude-sonnet-4-5" {
		t.Errorf("Model clobbered: got %q, want %q", got.Model, "claude-sonnet-4-5")
	}
	// LastBranchSHA should be updated (we explicitly set it)
	if got.LastBranchSHA != "fresh-sha-from-review" {
		t.Errorf("LastBranchSHA = %q, want %q", got.LastBranchSHA, "fresh-sha-from-review")
	}
}
...
func TestIsDormant(t *testing.T) {
	tests := []struct {
		...
		{"neither flag", Agent{}, false},
		{"WaitingForPR only", Agent{WaitingForPR: true}, true},
		{"WaitingForVerification only", Agent{WaitingForVerification: true}, true},
		{"both flags", Agent{WaitingForPR: true, WaitingForVerification: true}, true},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := tt.agent.IsDormant(); got != tt.expected {
				t.Errorf("IsDormant() = %v, want %v", got, tt.expected)
			}
		})
	}
}

func TestClearDormancy(t *testing.T) {
	agent := Agent{
		WaitingForPR:                true,
		WaitingForPRSince:           time.Now(),
		WaitingForVerification:      true,
		WaitingForVerificationSince: time.Now(),
	}

	agent.ClearDormancy()

	if agent.WaitingForPR {
		t.Error("WaitingForPR should be false after ClearDormancy")
	}
	if !agent.WaitingForPRSince.IsZero() {
		t.Error("WaitingForPRSince should be zero after ClearDormancy")
	}
	if agent.WaitingForVerification {
		t.Error("WaitingForVerification should be false after ClearDormancy")
	}
	if !agent.WaitingForVerificationSince.IsZero() {
		t.Error("WaitingForVerificationSince should be zero after ClearDormancy")
	}
	if agent.IsDormant() {
		t.Error("IsDormant() should be false after ClearDormancy")
	}
}

func TestModifyAgentDoesNotClobberWaitingForVerification(t *testing.T) {
	tmpDir := t.TempDir()
	s := New(filepath.Join(tmpDir, "state.json"))

	s.AddRepo("repo", &Repository{
		GithubURL:   "https://github.com/test/repo",
		SessionName: "test",
		Agents:      make(map[string]Agent),
	})
	s.AddAgent("repo", "worker-1", Agent{
		Type:                        AgentTypeWorker,
		WindowName:                  "worker-1",
		Task:                        "Build feature",
		WaitingForVerification:      true,
	...
	// Simulate a nudge loop updating only NudgeCount
	if err := s.ModifyAgent("repo", "worker-1", func(a *Agent) {
		a.NudgeCount = 5
		a.LastBranchSHA = "abc123"
	}); err != nil {
		t.Fatalf("ModifyAgent() error: %v", err)
	}

	got, exists := s.GetAgent("repo", "worker-1")
	if !exists {
		t.Fatal("Agent should exist")
	}
	if !got.WaitingForVerification {
		t.Error("WaitingForVerification clobbered: got false, want true")
	}
	if got.WaitingForVerificationSince.IsZero() {
		t.Error("WaitingForVerificationSince clobbered: got zero")
	}
	if !got.WaitingForPR {
		t.Error("WaitingForPR clobbered: got false, want true")
	}
	if got.NudgeCount != 5 {
		t.Errorf("NudgeCount = %d, want 5", got.NudgeCount)
	}
}
