...
const (
	...
	// TrackModeAuthor tracks only PRs where the oat user is the author
	TrackModeAuthor TrackMode = "author"
	// TrackModeAssigned tracks only PRs where the oat user is assigned
...
type TaskHistoryEntry struct {
	...
	Model         string     `json:"model,omitempty"`          // LLM model used for this task
	...
	InputTokens   int64      `json:"input_tokens,omitempty"`   // Snapshot of cumulative input tokens at completion
	OutputTokens  int64      `json:"output_tokens,omitempty"`  // Snapshot of cumulative output tokens at completion
	TotalTokens   int64      `json:"total_tokens,omitempty"`   // InputTokens + OutputTokens at completion
}
...
type Agent struct {
	...
	WaitingForPRSince           time.Time `json:"waiting_for_pr_since,omitempty"`           // When the worker started waiting for PR
	WaitingForVerification      bool      `json:"waiting_for_verification,omitempty"`       // Worker is dormant waiting for verification verdict
	WaitingForVerificationSince time.Time `json:"waiting_for_verification_since,omitempty"` // When the worker started waiting for verification
	PRNumber                    int       `json:"pr_number,omitempty"`                      // PR number being monitored
	// Verification agent fields (workers only)
	VerificationAgent    string    `json:"verification_agent,omitempty"`     // Name of linked verification agent
	VerificationStatus   string    `json:"verification_status,omitempty"`    // "" | "pending" | "approved" | "rejected"
	VerifiedCommitSHA    string    `json:"verified_commit_sha,omitempty"`    // Commit SHA the verdict applies to
	VerificationAt       time.Time `json:"verification_at,omitempty"`        // When verdict was set
	VerificationReason   string    `json:"verification_reason,omitempty"`    // Reason text (especially for rejection)
	LastPRCommentID      int64     `json:"last_pr_comment_id,omitempty"`     // Last seen PR comment ID (for detecting new activity)
	LastWakeReason       string    `json:"last_wake_reason,omitempty"`       // Wake message sent when transitioning from dormant; re-sent on first nudge
...
// PR resolution or waiting for a verification verdict).
func (a Agent) IsDormant() bool {
	return a.WaitingForPR || a.WaitingForVerification
}

// ClearDormancy resets all dormancy flags and timestamps.
func (a *Agent) ClearDormancy() {
	a.WaitingForPR = false
	a.WaitingForPRSince = time.Time{}
...
// ModifyAgent atomically reads the current agent state, applies the given
// function, and writes the result back. This avoids read-modify-write races
// where a caller holding a stale copy could overwrite concurrent updates
// (e.g., a nudge loop clobbering WaitingForPR/WaitingForVerification set by handleAgentWaiting).
func (s *State) ModifyAgent(repoName, agentName string, fn func(*Agent)) error {
	s.mu.Lock()
	defer s.mu.Unlock()
...
// ModifyRepo atomically reads a repository's state, applies the given
// function, and writes the result back. This avoids read-modify-write races
// in the same way as ModifyAgent.
func (s *State) ModifyRepo(repoName string, fn func(*Repository)) error {
	s.mu.Lock()
	defer s.mu.Unlock()
...
// ListAgents returns all agent names for a repository, sorted alphabetically
// for deterministic ordering (Go map iteration is randomized).
...
// SetRepoIdleMode sets whether a repository is in idle mode (daemon has paused nudges for that repo).
func (s *State) SetRepoIdleMode(repoName string, idle bool) error {
	s.mu.Lock()
	defer s.mu.Unlock()
