...
// TestGenerateSessionID is now in pkg/agent/agent_test.go
...
func TestGenerateDocumentation(t *testing.T) {
	...
	if !strings.Contains(docs, "# OAT CLI Reference") {
...
func setupTestEnvironment(t *testing.T) (*CLI, *daemon.Daemon, func()) {
	...
	// Set test mode to skip agent startup
...
func TestCLIWorkCreateWithBackend(t *testing.T) {
	cli, d, cleanup := setupTestEnvironment(t)
	defer cleanup()
	...
	be := d.GetBackend()
	sessionName := "oat-test-repo"
	if err := be.CreateSession(context.Background(), sessionName); err != nil {
		t.Fatalf("Failed to create backend session: %v", err)
	}
	defer be.DestroySession(context.Background(), sessionName)
	...
	// In test mode (OAT_TEST_MODE=1), agent startup is skipped.
	// The backend creates agent processes atomically with agent start,
	// so no backend process is expected in test mode.
	// Verify the agent was registered in state instead.
...
// TestWriteWorkerPromptFileWithIssue verifies that --issue injects the GitHub issue line into the worker prompt
func TestWriteWorkerPromptFileWithIssue(t *testing.T) {
	tmpDir := t.TempDir()
	paths := config.NewTestPaths(tmpDir)
	repoPath := filepath.Join(paths.ReposDir, "test-repo")
	if err := os.MkdirAll(repoPath, 0755); err != nil {
		t.Fatalf("Failed to create repo dir: %v", err)
	}
	cli := NewWithPaths(paths)

	path, err := cli.writeWorkerPromptFile(repoPath, "test-worker", WorkerConfig{
		IssueNumber: "42",
		IssueURL:    "https://github.com/owner/repo/issues/42",
	})
	if err != nil {
		t.Fatalf("writeWorkerPromptFile() failed: %v", err)
	}
	content, err := os.ReadFile(path)
	if err != nil {
		t.Fatalf("Failed to read prompt file: %v", err)
	}
	if !strings.Contains(string(content), "GitHub issue for this task: #42") {
		t.Errorf("Prompt file should contain 'GitHub issue for this task: #42', got:\n%s", string(content))
	}
	if !strings.Contains(string(content), "https://github.com/owner/repo/issues/42") {
		t.Errorf("Prompt file should contain issue URL, got:\n%s", string(content))
	}
}

func TestInferRepoFromCwd(t *testing.T) {
	// Create temp directories to simulate oat structure
	tmpDir := t.TempDir()
...
func TestHasPathPrefix(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			path:   "/home/user/.oat/wts-backup/repo/agent",
			prefix: "/home/user/.oat/wts",
			want:   false,
		},
		{
			name:   "wts should match wts",
			path:   "/home/user/.oat/wts/repo/agent",
			prefix: "/home/user/.oat/wts",
			want:   true,
		},
		{
...
func TestInferAgentContext(t *testing.T) {
	...
	// Test main repo dir - agent name depends on backend context, so just verify repo is found
	t.Run("in main repo dir returns repo", func(t *testing.T) {
		...
		// Agent name may vary based on backend context - don't assert specific value
	})
}
...
func TestCLIRemoveWorkerWithBackend(t *testing.T) {
	cli, d, cleanup := setupTestEnvironment(t)
	defer cleanup()
	...
	// Create backend session
	be := d.GetBackend()
	sessionName := "oat-test-rm"
	if err := be.CreateSession(context.Background(), sessionName); err != nil {
		t.Fatalf("Failed to create backend session: %v", err)
	}
	defer be.DestroySession(context.Background(), sessionName)

	// Start a test agent process for the worker
	handle, err := be.StartAgent(context.Background(), backend_pkg.AgentConfig{
		SessionName: sessionName,
	...
	_ = handle
	defer be.StopAgent(context.Background(), sessionName, "test-worker")
	...
	// Remove worker
	rmErr := cli.Execute([]string{"work", "rm", "test-worker", "--repo", repoName})
	if rmErr != nil {
		t.Errorf("work rm failed: %v", rmErr)
	}
...
func TestSanitizeSessionName(t *testing.T) {
	tests := []struct {
		...
		{"my-repo", "oat-my-repo"},
		{"demos.expanso.io", "oat-demos-expanso-io"},
		{"repo.with.many.dots", "oat-repo-with-many-dots"},
		{"repo:with:colons", "oat-repo-with-colons"},
		{"repo with spaces", "oat-repo-with-spaces"},
		{"simple", "oat-simple"},
		{"repo/with/slashes", "oat-repo-with-slashes"},
		{"path/to/repo.git", "oat-path-to-repo-git"},
		{"repo\x00with\x1fnull", "oat-repowithnull"}, // control characters stripped
	}

	for _, tt := range tests {
		t.Run(tt.repoName, func(t *testing.T) {
			got := sanitizeSessionName(tt.repoName)
			if got != tt.want {
				t.Errorf("sanitizeSessionName(%q) = %q, want %q", tt.repoName, got, tt.want)
			}
		})
	}
}
...
`
	...
	repoAgentsDir := filepath.Join(repoPath, ".oat", "agents")
...
func TestGetAgentBinaryReturnsValue(t *testing.T) {
	cli, _, cleanup := setupTestEnvironment(t)
	defer cleanup()

	binary, err := cli.getAgentBinary()
	// May fail if oat-agent not installed, but shouldn't panic
	if err == nil && binary == "" {
		t.Error("getAgentBinary() returned empty string without error")
	}
}
...
func TestCLIAddWorkspace(t *testing.T) {
	...
	// Note: Full workspace creation requires a running backend and proper daemon state,
...
func TestCLIRemoveWorkspace(t *testing.T) {
	...
	// Note: Full workspace removal requires a running backend and proper daemon state,
	// which is tested in integration tests. Here we test the validation logic.
}
...
// TestGetAgentBinaryFunction tests the getAgentBinary function
func TestGetAgentBinaryFunction(t *testing.T) {
	cli, _, cleanup := setupTestEnvironment(t)
	defer cleanup()

	// This test checks that getAgentBinary uses exec.LookPath
	// If oat-agent is not installed, it returns an error
	binary, err := cli.getAgentBinary()
	if err != nil {
		// This is expected in CI environments where oat-agent is not installed
		// The error should be a AgentBinaryNotFound error
		if !strings.Contains(err.Error(), "oat-agent") {
			t.Errorf("getAgentBinary() error should mention oat-agent: %v", err)
		}
	} else {
		// If we found it, the path should be non-empty
		if binary == "" {
			t.Error("getAgentBinary() returned empty path without error")
		}
	}
}

// TestAgentSubcommandRouting verifies that all "oat agent" subcommands are
// registered and routable. This catches the key collision regression where
// c.rootCmd.Subcommands["agent"] was overwritten by restartAgentInContext,
// destroying the subcommand tree (complete, waiting, etc.).
func TestAgentSubcommandRouting(t *testing.T) {
	tmpDir, err := os.MkdirTemp("", "cli-routing-test-*")
	if err != nil {
		t.Fatal(err)
	}
	defer os.RemoveAll(tmpDir)
	...
	agentCmd, exists := cli.rootCmd.Subcommands["agent"]
	if !exists {
		t.Fatal("rootCmd.Subcommands[\"agent\"] not registered")
	}

	if agentCmd.Run == nil {
		t.Error("agent command has nil Run (restartAgentInContext fallback missing)")
	}
	...
	for _, name := range expectedSubcommands {
		sub, ok := agentCmd.Subcommands[name]
		if !ok {
			t.Errorf("agent subcommand %q not registered (key collision overwrote tree?)", name)
			continue
		}
		if sub.Run == nil {
			t.Errorf("agent subcommand %q has nil Run handler", name)
		}
	}
}
...
func TestExtractOwnerRepo(t *testing.T) {
	tests := []struct {
		...
		{"https URL", "https://github.com/Root-IO-Labs/open-agent-teams", "Root-IO-Labs/open-agent-teams"},
		{"https URL with .git", "https://github.com/Root-IO-Labs/open-agent-teams.git", "Root-IO-Labs/open-agent-teams"},
		{"http URL", "http://github.com/owner/repo", "owner/repo"},
		{"SSH URL", "git@github.com:Root-IO-Labs/open-agent-teams.git", "Root-IO-Labs/open-agent-teams"},
		{"SSH URL without .git", "git@github.com:owner/repo", "owner/repo"},
		{"git protocol", "git://github.com/owner/repo.git", "owner/repo"},
		{"case-insensitive prefix", "HTTPS://GITHUB.COM/Owner/Repo", "Owner/Repo"},
		{"non-GitHub URL fallback", "https://gitlab.com/owner/repo", "https://gitlab.com/owner/repo"},
		{"empty string", "", ""},
		{"whitespace trimmed", "  https://github.com/owner/repo  ", "owner/repo"},
	}
...
func TestIssueNumberFromTaskRegex(t *testing.T) {
	tests := []struct {
		name     string
		task     string
		expected string
	}{
		{"benchmark format", "Work issue #5: Implement: JSON Storage in repo Root-IO-Labs/oat-bench", "5"},
		{"uppercase Issue", "Issue #42: Fix login bug", "42"},
		{"lowercase issue", "issue #7: Add tests", "7"},
		{"no space before hash", "issue#12 something", "12"},
		{"large number", "Work issue #123: Refactor database layer", "123"},
		{"no issue reference", "Add unit tests for auth module", ""},
		{"hash without issue keyword", "Fix #99 login bug", ""},
		{"empty string", "", ""},
		{"issue in middle of text", "Please work on issue #15 and report back", "15"},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			match := issueNumberFromTaskRegex.FindStringSubmatch(tt.task)
			var result string
			if len(match) > 1 {
				result = match[1]
			}
			if result != tt.expected {
				t.Errorf("issueNumberFromTaskRegex(%q) = %q, want %q", tt.task, result, tt.expected)
			}
		})
	}
}

// TestFormatAge covers the human-age formatter used by `oat status --tokens`
// for the LAST UPDATE column.
func TestFormatAge(t *testing.T) {
	tests := []struct {
		name    string
		t       time.Time
		wantFmt string // substring — exact seconds are flaky due to wall time
		wantEq  string // exact match (only for "—" and hour cases)
	}{
		{
			name:   "zero time renders em dash",
			t:      time.Time{},
			wantEq: "—",
		},
		{
			name:    "sub-minute renders seconds",
			t:       time.Now().Add(-30 * time.Second),
			wantFmt: "s", // "30s" / "29s" etc.
		},
		{
			name:    "sub-hour renders mNsS",
			t:       time.Now().Add(-(2*time.Minute + 34*time.Second)),
			wantFmt: "2m",
		},
		{
			name:    "hour-plus renders hNmM",
			t:       time.Now().Add(-(1*time.Hour + 5*time.Minute)),
			wantFmt: "1h",
		},
		{
			name:    "negative delta clamps to zero",
			t:       time.Now().Add(1 * time.Minute), // future — delta < 0
			wantFmt: "0s",
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantEq != "" {
				if got != tt.wantEq {
					t.Errorf("formatAge = %q, want %q", got, tt.wantEq)
				}
				return
			}
			if !strings.Contains(got, tt.wantFmt) {
				t.Errorf("formatAge = %q, want substring %q", got, tt.wantFmt)
			}
		})
	}
}

// TestFormatAge_StaleHighlight verifies the staleness-threshold contract
// that systemStatusTokens relies on: a non-zero timestamp older than
// staleTokenAge is considered stale; zero or fresh timestamps are not.
func TestFormatAge_StaleHighlight(t *testing.T) {
	cases := []struct {
		name      string
		t         time.Time
		wantStale bool
	}{
		{"zero is never stale", time.Time{}, false},
		{"30s ago is fresh", time.Now().Add(-30 * time.Second), false},
		{"4m ago is fresh", time.Now().Add(-4 * time.Minute), false},
		{"6m ago is stale", time.Now().Add(-6 * time.Minute), true},
		{"1h ago is stale", time.Now().Add(-1 * time.Hour), true},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			// This is exactly the predicate systemStatusTokens uses.
			stale := !tc.t.IsZero() && time.Since(tc.t) > staleTokenAge
			if stale != tc.wantStale {
				t.Errorf("stale(%v) = %v, want %v", tc.t, stale, tc.wantStale)
			}
		})
	}
}
