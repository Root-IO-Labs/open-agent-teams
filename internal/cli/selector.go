...
func formatAgentStatusCell(status string) format.ColoredCell {
	...
	case "waiting for PR":
		return format.ColorCell(format.ColoredStatus(format.StatusWaitingForPR), nil)
	case "waiting for verification":
		return format.ColorCell(format.ColoredStatus(format.StatusWaitingForVerification), nil)
	case "stopped":
