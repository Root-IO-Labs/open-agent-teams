...
func TestGetDefaultPromptContent(t *testing.T) {
	...
	if !strings.Contains(supervisorPrompt, "oat message send") {
	...
	if !strings.Contains(workspacePrompt, "oat message send") {
...
func TestLoadCustomPrompt(t *testing.T) {
	...
	tmpDir, err := os.MkdirTemp("", "oat-prompts-test-*")
	if err != nil {
	...
	oatDir := filepath.Join(tmpDir, ".oat")
	...
	t.Run("with custom supervisor prompt", func(t *testing.T) {
		...
		promptPath := filepath.Join(oatDir, "SUPERVISOR.md")
	...
	t.Run("with custom worker prompt", func(t *testing.T) {
		...
		promptPath := filepath.Join(oatDir, "WORKER.md")
		if err := os.WriteFile(promptPath, []byte(customContent), 0644); err != nil {
			t.Fatalf("failed to write custom prompt: %v", err)
		}
	...
	t.Run("with custom merge-queue prompt", func(t *testing.T) {
		...
		promptPath := filepath.Join(oatDir, "MERGE-QUEUE.md")
		if err := os.WriteFile(promptPath, []byte(customContent), 0644); err != nil {
			t.Fatalf("failed to write custom prompt: %v", err)
		}
	...
	t.Run("with custom workspace prompt", func(t *testing.T) {
		...
		promptPath := filepath.Join(oatDir, "WORKSPACE.md")
		if err := os.WriteFile(promptPath, []byte(customContent), 0644); err != nil {
			t.Fatalf("failed to write custom prompt: %v", err)
		}
	...
	t.Run("with custom review prompt", func(t *testing.T) {
		...
		promptPath := filepath.Join(oatDir, "REVIEW.md")
		if err := os.WriteFile(promptPath, []byte(customContent), 0644); err != nil {
			t.Fatalf("failed to write custom prompt: %v", err)
		}
...
func TestGetSlashCommandsPromptContainsCLICommands(t *testing.T) {
	...
	statusCommands := []struct {
		...
		{"oat daemon status", "/status should include daemon status check"},
	...
	workersCommands := []struct {
		...
		{"oat worker list", "/workers should include worker list command"},
	}
	...
	messagesCommands := []struct {
		...
		{"oat message list", "/messages should include list command"},
	}
...
func TestGetPromptIncludesSlashCommandsForHardcodedAgentTypes(t *testing.T) {
	...
	for _, agentType := range agentTypes {
		t.Run(string(agentType), func(t *testing.T) {
			...
			// Supervisor/workspace get globally scoped commands only (/status, /messages).
			// /refresh and /workers are for worktree and merge-queue agents respectively.
			expectedCommands := []string{"/status", "/messages"}
			for _, cmd := range expectedCommands {
				if !strings.Contains(prompt, cmd) {
			...
			if strings.Contains(prompt, "# /refresh") {
				t.Errorf("GetPrompt(%s) should not include /refresh for this agent type", agentType)
			}
			if strings.Contains(prompt, "# /workers") {
				t.Errorf("GetPrompt(%s) should not include /workers for this agent type", agentType)
			}
		})
	}
}
