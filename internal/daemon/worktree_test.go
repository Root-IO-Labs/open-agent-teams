...
func TestGetAgentBinaryPathWorktree(t *testing.T) {
	d, cleanup := setupTestDaemon(t)
	defer cleanup()
	...
	// If oat-agent is not installed, the test should skip
	if err != nil {
		t.Logf("Agent binary not found (expected in test environments): %v", err)
		return
	}
